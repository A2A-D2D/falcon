...
module tb_fft_debug_sign;
    ...
    always @(posedge clk) begin
        // When FFT is active (FC phase), print some info
        if (dut.st == 3) begin  // FC_FFT
            if (dut.u_fft.state != 0) begin  // Not IDLE
                $display("T=%0t FFT: state=%0d, stage=%0d, pair=%0d", 
                         $time, dut.u_fft.state, dut.u_fft.stage_idx, dut.u_fft.pair_idx);
            end
        end
    end

    // Debug HashToPoint output
    always @(posedge clk) begin
        if (dut.st == 2 && dut.htp_coeff_valid) begin  // HP phase
            $display("T=%0t HTP: coeff=%0d", $time, dut.htp_coeff);
        end
    end
    ...
    initial begin
        ...
        // Start signing (CASE=1: full pipeline)
        $display("=== Starting signing (CASE=1) ===");
        @(posedge clk);
        bus_cs = 1;
        bus_wr = 1;
        bus_addr = 16'h0008;
