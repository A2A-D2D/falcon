...
module tb_norm_check_debug;

    // Decode some s1 and s2 values to check norm
    reg [15:0] s1_vals [0:15];
    reg [15:0] s2_vals [0:15];
    reg signed [15:0] s1_centered [0:15];
    ...
    initial begin
        ...
        // Center-lift s1 values
        $display("=== s1 values (center-lifted) ===");
        s1_norm = 0;
        for (i = 0; i < 16; i = i + 1) begin
            if (s1_vals[i] > 16'd6144) begin
                s1_centered[i] = $signed(s1_vals[i]) - 16'sd12289;
            end else begin
                s1_centered[i] = $signed(s1_vals[i]);
            end
            $display("  s1[%0d] = %0d -> %0d (sq=%0d)", 
                     i, s1_vals[i], s1_centered[i], 
                     s1_centered[i] * s1_centered[i]);
            s1_norm = s1_norm + (s1_centered[i] * s1_centered[i]);
        end
        $display("  s1 partial norm = %0d", s1_norm);
        ...
        for (i = 0; i < 16; i = i + 1) begin
            $display("  s2[%0d] = %0d (sq=%0d)", 
                     i, $signed(s2_vals[i]), 
                     $signed(s2_vals[i]) * $signed(s2_vals[i]));
            s2_norm = s2_norm + ($signed(s2_vals[i]) * $signed(s2_vals[i]));
        end
        $display("  s2 partial norm = %0d", s2_norm);
        ...
        $display("  Estimated full norm (512 coeffs) = %0d", (s1_norm + s2_norm) * 32);
